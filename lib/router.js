...
const makeHandler = ({handler, input: actionInput, source = SOURCE_REMOTE, actionOptions = {}}) => {
    ...
    return async (req, res) => {
        ...
        try {
            ...
            logger.error(info);
            let message = !isProd ? info : req.translate('errors.genericError');
...
exports.registerAction = registerAction = (handler, opts) => {
    ...
    if (opts.route) {
        app[opts.route[0]].apply(app, [opts.route[1], ...middlewareList, makeHandler({
            handler,
            source: SOURCE_REMOTE,
            actionOptions: opts,
            ...(actions[name].input || {})
