...
const handle = async (req, res) => {
        ...
        let processMiddleware = (key = 0) => {
            middleware[key](req, res, async error => {
                ...
                if (++key < middleware.length) {
                    return processMiddleware(invokedAction, key);
                }
    ...
    if (config.batch && isBatchRequest) {
        ...
        await Promise.all(actions.map(i => runAction(i)));
        res._json.call(app, results);
        return;
    }
...
const makeHandler = ({handler, input: actionInput, source = 'remote'}) => {
    const {
        input: inputPlugin,
        axios,
        db,
