...
exports.handle = async (req, actionInput) => {
    ...
    for (const key in actionInput) {
        ...
        let lang = req.user && req.user.locale ? req.user.locale : config.get('plugins.i18n.defaultLocale');

        let validateField = validator.createFieldValidator(i18n.translator(lang).translate);
        let messages = await validateField(input[key], key, actionInput[key], input);
