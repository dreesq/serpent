...
exports.register = register = (name, Plugin) => {
     ...
     const isEnabled = plugin => (config && config.get) ? config.get(`plugins.${plugin}.enabled`) : true;

     typeof Plugin.init === 'function' && isEnabled(name) && Plugin.init(context);
     typeof Plugin === 'function' ? (Plugin = Plugin(context)) : null;
